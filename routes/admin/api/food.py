...
def get_food():
  ...
  cursor.execute("""
    SELECT
      ...
      f.facilities,
      f.createTime,
      COUNT(DISTINCT fi.image) AS imgCount,
      GROUP_CONCAT(DISTINCT ft.name) AS topics
    FROM food AS f
    LEFT JOIN food_image AS fi
      ON f.id = fi.foodId
    LEFT JOIN food_topic AS ft
      ON f.id = ft.foodId
    GROUP BY f.id
    ORDER BY f.id DESC
  """)
...
# 刪除單個圖片
@bp.delete('/<int:foodId>/img/<imgFileName>')
@api_manager_required
def delete_img(foodId, imgFileName):
  # =========================
  # 檔名安全性檢查
  # =========================
...
def delete_food(foodId):
  ...
  try:
    ...
    # 刪除分類
    cursor.execute(
        "DELETE FROM food_topic WHERE foodId = ?",
        (foodId,)
    )

    # 刪除留言
    cursor.execute(
        "DELETE FROM food_comment WHERE foodId = ?",
        (foodId,)
    )

    # 刪除圖片資料
    cursor.execute(
        "DELETE FROM food_image WHERE foodId = ?",
        (foodId,)
    )
